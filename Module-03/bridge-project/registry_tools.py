...
def gleif_lookup(legal_name: str) -> str:
    ...
    try:
        ...
        url = f"https://api.gleif.org/api/v1/lei-records?{query_string}"
        ...
        for record in records:
            ...
            candidate_name = legal_name_data.get("name", "UNKNOWN")

            lei = attributes.get("lei", "UNKNOWN")
            ...
            is_exact = candidate_name.casefold() == legal_name.casefold()
            if is_exact:
                exact_matches += 1
            ...
            candidates.append(
                "\n".join(
                    [
                        ...
                        f"Exact name match: {'YES' if is_exact else 'NO'}",
                    ]
                )
            )

        header = (
            f"GLEIF candidates for '{legal_name}': "
            f"{len(candidates)} found, {exact_matches} exactly matching."
        )
